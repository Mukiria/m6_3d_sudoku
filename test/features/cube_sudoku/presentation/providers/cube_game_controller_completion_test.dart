...
Future<void> _awaitStatisticsLoaded(ProviderContainer container) async {
  ...
  if (value.hasError) {
    fail('statisticsProvider failed to load: ${value.error}\n${value.stackTrace}');
  }
}
...
void main() {
  ...
  test('completing all six faces flips isComplete exactly once, at the end, and feeds per-difficulty stats as each face completes', () async {
      ...
      for (var i = 0; i < empties.length; i++) {
        ...
        if (isLastCellOfFace) {
          ...
          expect(
            ...
            reason: 'each completed face should count immediately, not '
        ...
        if (isLastCellOfFace && isLastFace) {
          expect(after.isComplete, true, reason: 'cube should be complete after the 6th face');
        } else {
          expect(
            after.isComplete,
            false,
  ...
  test('a face failing at 3 mistakes never blocks the other five from completing', () async {
    final container = ProviderContainer(
      overrides: [
        storageServiceProvider.overrideWithValue(FakeStorageService()),
        audioServiceProvider.overrideWithValue(FakeAudioService()),
    ...
    expect(
      state.isComplete,
      false,
      reason: 'a failed face should permanently block cube completion until retried',
    );
  });
}
