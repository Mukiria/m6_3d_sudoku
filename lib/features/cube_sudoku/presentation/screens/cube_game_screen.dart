...
class _CubeGameScreenState extends ConsumerState<CubeGameScreen>
  ...
  void _onBrowseScaleUpdate(ScaleUpdateDetails details) {
    ...
    setState(() {
      ...
      _browsePitch = (_browsePitch - details.focalPointDelta.dy * 0.006)
  ...
  void _jumpToFace(CubeFace face) {
    ...
    if (_mode == _CubeMode.browse) {
      ...
      setState(() {
        ...
        _browsePitch = CubeGeometry.unwrappedTarget(
          _browsePitch,
          target.pitch,
        );
      });
                  ...
                  children: [
                    ...
                    if (!forceFlat) ...[
                      ...
                      _CircleIconButton(
                        ...
                        onTap:
                            () =>
                                ref.read(show3DCubeProvider.notifier).state =
                                    !manualShow3D,
                      ),
                      if (showingCube) ...[
                        const SizedBox(width: AppConstants.spacingSm),
                ...
                builder: (context, ref, _) {
                  final progress = ref.watch(
                    cubeGameControllerProvider.select(
                      (s) => (
                        activeFace:
                            _showingCubeExperience && _mode == _CubeMode.browse
                                ? CubeGeometry.mostFacingCamera(
                                  _browseYaw,
                                  _browsePitch,
                                )
  ...
  Widget _buildStillFace(
    ...
    final normalZ = CubeGeometry.worldNormal(
      face,
      _browseYaw,
      _browsePitch,
    ).z;
