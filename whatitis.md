...
- **Release status (as of 2026-09-26):** `1.0.0+4` (the generation/grading work below) is on the Play Console **internal testing** track and was **submitted for production review on 2026-09-26** (all countries, managed publishing off — it goes live automatically on approval; Google Play Games on PC opted out). Every Play Console answer is recorded in `store_listing/PLAY_CONSOLE_ANSWERS.md`. Uploads are still manual — CI upload needs the not-yet-configured `PLAY_SERVICE_ACCOUNT_JSON`. Version codes 1, 3 and 4 have been uploaded, so the next build must be `1.0.0+5` or higher. The branch work was merged into `main` via PR #1 (2026-09-26). The Play account is the **MSIXV organisation account** (tech@msixv.com, `play.google.com/console/u/2`), so no closed-test requirement applied. Store listing copy, the app-specific privacy policy draft and store artwork live in `store_listing/` (artwork in `store_listing/images/`, deliberately outside `assets/` so it isn't bundled into the app — only the logotype, mascot and launcher-icon source remain in `assets/images/`). `todolist.md` is the running go-live checklist.
