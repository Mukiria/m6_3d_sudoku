...
- **Release status (as of 2026-09-26):** `1.0.0+4` (the generation/grading work below) is live on the Play Console **internal testing** track; production isn't submitted yet. Uploads are still manual — CI upload needs the not-yet-configured `PLAY_SERVICE_ACCOUNT_JSON`. Version codes 1, 3 and 4 have been uploaded, so the next build must be `1.0.0+5` or higher. The branch work was merged into `main` via PR #1 (2026-09-26). The Play account is an organisation account, so the 12-tester / 14-day closed-test rule shouldn't apply — production can be submitted directly once Play Console setup is complete (still to confirm in Play Console). Store listing copy, the app-specific privacy policy draft and store artwork live in `store_listing/` (artwork in `store_listing/images/`, deliberately outside `assets/` so it isn't bundled into the app — only the logotype, mascot and launcher-icon source remain in `assets/images/`). `todolist.md` is the running go-live checklist.
