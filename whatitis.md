...
- **`applicationId` vs. Play Store listing mismatch — unresolved, needs owner confirmation:** `android/app/build.gradle.kts` declares `applicationId "com.msixv.m6sudoku"`, but `README.md`/`RELEASE_NOTES.md` link the live Play Store listing as `com.m6.sudoku` (also §1's assumed bundle ID above, which came from those same docs — treat that as unverified against the actual gradle config). One of the two is stale. This blocks safely automating a Play Console upload until confirmed — see `docs/DEPLOYMENT.md` §0/§8.
